...
fn query_recent_valid_price(
    ...
    if marketmap_currency_pairs_response
        .market_map
        .markets
        .contains_key(&base_symbol.clone())
        == false
    {
        StdError::generic_err(format!(
    ...
    if (current_block_height - oracle_price_response.price.block_height.unwrap()) > max_blocks_old.u64() {
        StdError::generic_err(format!(
...
fn query_recent_valid_prices(
    ...
    let currency_pairs: Vec<CurrencyPair> = base_symbols
        .iter()
        .map(|symbol| CurrencyPair {
    ...
    let _ = currency_pairs.iter().map(|curr_pair| {
        if oracle_currency_pairs_response
            .currency_pairs
            .contains(curr_pair)
            == false
        {
            StdError::generic_err(format!(
                "Market {0}, {1} not found in x/oracle module",
                curr_pair.base, curr_pair.quote,
            ));
        }
    });
    ...
    let _ = currency_pairs.iter().map(|curr_pair| {
        ...
        {
            StdError::generic_err(format!(
                "Market {0}, {1} not enabled in x/oracle module",
                curr_pair.base, curr_pair.quote,
            ));
        }
    });
    ...
    let oracle_price_responses: Vec<GetPriceResponse> = currency_pairs
        .iter()
        .map(|curr_pair| {
            deps.querier
                .query(
    ...
    let _ = oracle_price_responses
        .iter()
        .enumerate()
        .map(|(index, price_response)| {
            if (current_block_height - price_response.price.block_height.unwrap()) > max_blocks_old.u64() {
                StdError::generic_err(format!(
                    "Market {0}, {1} not enabled in x/oracle module",
                    currency_pairs[index].base, currency_pairs[index].quote,
                ));
            }
        });

    // 4. fetch the price from x/oracle module
    let market_prices: Vec<Int128> = oracle_price_responses
        .iter()
        .map(|price_response| price_response.price.price)
        .collect();
    ...
    // 6. return the price as response with proper metadata
    Ok(to_json_binary(&GetPricesResponse {
